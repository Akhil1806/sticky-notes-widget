...
public class StickyNoteWidget extends AppWidgetProvider {
    ...
    private static CharSequence parseHtmlContent(String html) {
        ...
        clean = clean.replaceAll("(?i)<label[^>]*>.*?</label>", "");
        clean = clean.replaceAll("(?i)<input[^>]*>", "");

        // 2. Convert Tiptap Task items
        clean = clean.replaceAll("(?i)<li[^>]*data-checked=[\"']true[\"'][^>]*>(.*?)</li>", "<div>\u2611 <s>$1</s></div>");
        clean = clean.replaceAll("(?i)<li[^>]*data-checked=[\"']false[\"'][^>]*>(.*?)</li>", "<div>\u2610 $1</div>");

        // 3. Convert Quill Task items
        clean = clean.replaceAll("(?i)<li[^>]*data-list=[\"']checked[\"'][^>]*>(.*?)</li>", "<div>\u2611 <s>$1</s></div>");
        clean = clean.replaceAll("(?i)<li[^>]*data-list=[\"']unchecked[\"'][^>]*>(.*?)</li>", "<div>\u2610 $1</div>");

        // 4. Unwrap inner paragraphs to avoid double breaks
        clean = clean.replaceAll("(?i)<p>(.*?)</p>", "$1<br>");
